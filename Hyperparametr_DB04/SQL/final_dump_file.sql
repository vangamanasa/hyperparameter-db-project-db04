...
--
-- Temporary view structure for view `default_hp_val`
--

DROP TABLE IF EXISTS `default_hp_val`;
/*!50001 DROP VIEW IF EXISTS `default_hp_val`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8mb4;
/*!50001 CREATE VIEW `default_hp_val` AS SELECT 
 1 AS `HP04`,
 1 AS `hyper_def_values`,
 1 AS `algorithm_name`*/;
...
INSERT INTO `default_values` VALUES ('alpha','',1,'Y'),('categorical_encoding','AUTO',2,'N'),('categorical_encoding','AUTO',3,'Y'),('categorical_encoding','AUTO',4,'Y'),('col_sample_rate','1',2,'N'),('col_sample_rate_per_tree','1',2,'Y'),('distribution','AUTO',2,'N'),('histogram_type','AUTO',2,'Y'),('huber_alpha','0.9',2,'N'),('lambda','',1,'Y'),('learn_rate','0.1',2,'Y'),('learn_rate_annealing','1',2,'Y'),('max_abs_leafnode_pred','1.7976931348623157e+308',2,'N'),('max_depth','5',2,'Y'),('min_rows','10',2,'Y'),('min_split_improvement','1.00E-05',2,'N'),('missing_values_handling','MeanImputation',1,'Y'),('mtries','-1',3,'Y'),('mtries','-1',4,'N'),('ntrees','50',2,'Y'),('pred_noise_bandwidth','0',2,'Y'),('quantile_alpha','0.5',2,'N'),('sample_rate','1',2,'Y'),('seed','-1',1,'Y'),('standardize','TRUE',1,'N'),('theta','1.00E-10',1,'Y'),('tweedie','1.5',2,'N'),('tweedie_link_power','1',1,'Y'),('tweedie_variance_power','0',1,'Y');
...
--
-- Temporary view structure for view `list_of_datasets`
--

DROP TABLE IF EXISTS `list_of_datasets`;
/*!50001 DROP VIEW IF EXISTS `list_of_datasets`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8mb4;
/*!50001 CREATE VIEW `list_of_datasets` AS SELECT 
 1 AS `dataset_id`,
 1 AS `dataset_name`,
 1 AS `domain_name`,
 1 AS `problem_type`*/;
...
--
-- Temporary view structure for view `model_runtime`
--

DROP TABLE IF EXISTS `model_runtime`;
/*!50001 DROP VIEW IF EXISTS `model_runtime`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8mb4;
/*!50001 CREATE VIEW `model_runtime` AS SELECT 
 1 AS `modelId`,
 1 AS `run_time`*/;
SET character_set_client = @saved_cs_client;

--
-- Temporary view structure for view `model_with_no_hyperparameter`
--

DROP TABLE IF EXISTS `model_with_no_hyperparameter`;
/*!50001 DROP VIEW IF EXISTS `model_with_no_hyperparameter`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8mb4;
/*!50001 CREATE VIEW `model_with_no_hyperparameter` AS SELECT 
 1 AS `auc`,
 1 AS `logloss`,
 1 AS `mean_per_class_error`,
 1 AS `rmse`,
 1 AS `mse`,
 1 AS `class_id`,
 1 AS `leader_board_modelId`*/;
SET character_set_client = @saved_cs_client;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP FUNCTION IF EXISTS `hp_min_max` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` FUNCTION `hp_min_max`(parameter varchar(100) ) RETURNS varchar(100) CHARSET utf8mb4
    DETERMINISTIC
BEGIN
...
SELECT 'learn_rate' as hp_name into out1;
SELECT min(learn_rate) as minimum into out2 from hp04.model_gbm;
SELECT max(learn_rate) as maximum into out3 from hp04.model_gbm; 

ELSEIF(parameter = 'ntrees') THEN
SELECT 'ntrees' as hp_name into out1;
SELECT min(ntrees) as minimum into out2 from hp04.model_gbm;
SELECT max(ntrees) as maximum into out3 from hp04.model_gbm; 
END IF;

RETURN concat_ws('|',out1, out2,out3);
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP FUNCTION IF EXISTS `run_model_count` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` FUNCTION `run_model_count`(runTime INT, model_name varchar(100)) RETURNS int(11)
    DETERMINISTIC
BEGIN

DECLARE model_count INT;

IF(model_name = 'GBM') THEN
select count(*) INTO model_count from hp04.leader_board as a 
INNER JOIN hp04.run_time as b ON a.run_time_runId = b.runId
INNER JOIN hp04.dataset as c ON c.dataset_id = b.dataset_dataset_id 
WHERE b.run_time = (select run_time from hp04.run_time where run_time = runTime)
AND modelId LIKE 'GBM%';

ELSEIF(model_name = 'GLM') THEN
select count(*) INTO model_count from hp04.leader_board as a 
INNER JOIN hp04.run_time as b ON a.run_time_runId = b.runId
INNER JOIN hp04.dataset as c ON c.dataset_id = b.dataset_dataset_id 
WHERE b.run_time = (select run_time from hp04.run_time where run_time = runTime)
AND modelId LIKE 'GLM%';

ELSEIF(model_name = 'DeepLearning') THEN
select count(*) INTO model_count from hp04.leader_board as a 
INNER JOIN hp04.run_time as b ON a.run_time_runId = b.runId
INNER JOIN hp04.dataset as c ON c.dataset_id = b.dataset_dataset_id 
WHERE b.run_time = (select run_time from hp04.run_time where run_time = runTime)
AND modelId LIKE 'DeepLearning%';

ELSEIF(model_name = DRF) THEN
select count(*) INTO model_count from hp04.leader_board as a 
INNER JOIN hp04.run_time as b ON a.run_time_runId = b.runId
INNER JOIN hp04.dataset as c ON c.dataset_id = b.dataset_dataset_id 
WHERE b.run_time = (select run_time from hp04.run_time where run_time = runTime)
AND modelId LIKE 'DRF%';

ELSEIF(model_name = 'XRT') THEN
select count(*) INTO model_count from hp04.leader_board as a 
INNER JOIN hp04.run_time as b ON a.run_time_runId = b.runId
INNER JOIN hp04.dataset as c ON c.dataset_id = b.dataset_dataset_id 
WHERE b.run_time = (select run_time from hp04.run_time where run_time = runTime)
AND modelId LIKE 'XRT%';
END IF;
RETURN model_count;
END ;;
DELIMITER ;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `Best_for_domain` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `dataset_run_model` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `dataset_run_model`(IN runTime INT,
IN datasetid INT)
BEGIN

select a.modelId,b.run_time,c.dataset_id from hp04.leader_board as a 
INNER JOIN hp04.run_time as b ON a.run_time_runId = b.runId
INNER JOIN hp04.dataset as c ON c.dataset_id = b.dataset_dataset_id 
WHERE b.run_time = (select run_time from hp04.run_time where run_time = runTime)
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `hp_min_max_proc` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `hp_range` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
--
-- Final view structure for view `default_hp_val`
--
...
/*!50001 VIEW `default_hp_val` AS select `a`.`HP04` AS `HP04`,`a`.`hyper_def_values` AS `hyper_def_values`,`b`.`algorithm_name` AS `algorithm_name` from (`default_values` `a` join `algorithm` `b` on((`a`.`algorithm_algo_id` = `b`.`algo_id`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `list_of_datasets`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `list_of_datasets` AS select `dataset`.`dataset_id` AS `dataset_id`,`dataset`.`dataset_name` AS `dataset_name`,`dataset`.`domain_name` AS `domain_name`,`dataset`.`problem_type` AS `problem_type` from `dataset` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `model_runtime`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `model_runtime` AS select `a`.`modelId` AS `modelId`,`b`.`run_time` AS `run_time` from (`leader_board` `a` join `run_time` `b`) where (`a`.`run_time_runId` = `b`.`runId`) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `model_with_no_hyperparameter`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `model_with_no_hyperparameter` AS select `classification_metrics`.`auc` AS `auc`,`classification_metrics`.`logloss` AS `logloss`,`classification_metrics`.`mean_per_class_error` AS `mean_per_class_error`,`classification_metrics`.`rmse` AS `rmse`,`classification_metrics`.`mse` AS `mse`,`classification_metrics`.`class_id` AS `class_id`,`classification_metrics`.`leader_board_modelId` AS `leader_board_modelId` from `classification_metrics` where (`classification_metrics`.`leader_board_modelId` like 'Stack%') */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2019-04-25 12:27:38
